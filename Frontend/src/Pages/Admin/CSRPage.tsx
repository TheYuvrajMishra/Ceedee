...
const AdminCSR = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to ${editingCsr ? "update" : "add"} CSR initiative`
        );
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete CSR initiative."
      );
    } finally {
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                            ...
                            {csr.budget && (
                              <span>💰 ${csr.budget.toLocaleString()}</span>
                            )}
                            {csr.startDate && (
                              <span>
                                📅 {formatDateForDisplay(csr.startDate)}
                              </span>
                            )}
                          </div>
                        ...
                        <div className="flex items-center space-x-2 ml-4">
                          ...
                          >
                            {expandedInitiatives.has(csr._id) ? "Hide" : "View"}{" "}
                            Details
                          </button>
                        </div>
                      </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                              ...
                              <div className="space-y-1 text-sm text-gray-600">
                                {csr.startDate && (
                                  <p>
                                    Start: {formatDateForDisplay(csr.startDate)}
                                  </p>
                                )}
                                {csr.endDate && (
                                  <p>
                                    End: {formatDateForDisplay(csr.endDate)}
                                  </p>
                                )}
                                {!csr.startDate && !csr.endDate && (
                                  <p>No timeline specified</p>
                                )}
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              {csr.contactPerson?.name ||
                              csr.contactPerson?.email ||
                              csr.contactPerson?.phone ? (
                                <div className="text-sm text-gray-600 space-y-1">
                                  {csr.contactPerson.name && (
                                    <p>{csr.contactPerson.name}</p>
                                  )}
                                  {csr.contactPerson.email && (
                                    <p>{csr.contactPerson.email}</p>
                                  )}
                                  {csr.contactPerson.phone && (
                                    <p>{csr.contactPerson.phone}</p>
                                  )}
                                </div>
                              ) : (
                                <p className="text-sm text-gray-600">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-900">
                  {editingCsr
                    ? "Edit CSR Initiative"
                    : "Create New CSR Initiative"}
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                  {editingCsr
                    ? "Update the initiative details below"
                    : "Fill in the details to create a new CSR initiative"}
                </p>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Are you sure you want to delete this CSR initiative? This
